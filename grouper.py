...
def slice_list(lst: List[Any], n: int) -> List[List[Any]]:
    ...
    #     for items in lst:
    #
    #     len(new_small_lsts) == n
    #     for items in lst:
    #         item = item + lst[]
    #         new_lst.append(lst[:n])

    # we need a big list comprised of small lists that are size n
    # make a big list
    # for n items in lst, make a list of size n
    # append the list of size n to the big list

    # new_lst = []
    # for item in lst:

    # len(lst_in_new_lst) == n
    # new_lst = new_lst +
...
class Group:
    ...
    def __contains__(self, member: Student) -> bool:
        ...
        for participant in self._members:
            if id(participant) == id(member):
                return True
        return False
